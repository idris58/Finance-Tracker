...
export class LocalStorage implements IStorage {
  ...
  async createTransaction(transaction: Omit<Transaction, 'id'>): Promise<Transaction> {
    return await db.transaction('rw', [db.categories, db.accounts, db.transactions], async () => {
      ...
      if (transaction.accountId) {
        ...
        if (account) {
          ...
          await db.accounts.update(account.id!, { balance: toMoneyString(newBalance) });
        }
      }
      ...
      if (transaction.loanSettlementAccountId && (!transaction.settlements || transaction.settlements.length === 0)) {
        const account = await db.accounts.get(transaction.loanSettlementAccountId);
        if (account) {
      ...
      return {
        ...created,
        date: created.date instanceof Date ? created.date : new Date(created.date),
        settlementDate: created.settlementDate instanceof Date ? created.settlementDate : (created.settlementDate ? new Date(created.settlementDate) : null),
        settlements: Array.isArray(created.settlements) ? created.settlements.map((s) => ({
          ...s,
          date: s.date instanceof Date ? s.date : new Date(s.date),
        })) : [],
      };
  ...
  async updateTransaction(id: number, updates: Partial<Omit<Transaction, 'id'>>): Promise<Transaction> {
    return await db.transaction('rw', [db.categories, db.accounts, db.transactions], async () => {
      ...
      if (merged.categoryId && merged.categoryId !== existing.categoryId) {
        const cat = await db.categories.get(merged.categoryId);
        if (cat) {
      ...
      if (merged.accountId) {
        const account = await db.accounts.get(merged.accountId);
        if (account) {
      ...
      if (existing.accountId && existing.accountId === merged.accountId) {
        ...
        if (account) {
          ...
          await db.accounts.update(account.id!, { balance: toMoneyString(next) });
        }
      } else {
        if (existing.accountId) {
          const oldAccount = await db.accounts.get(existing.accountId);
          if (oldAccount) {
            const next = roundMoney(Number(oldAccount.balance || 0) - oldDelta);
            await db.accounts.update(oldAccount.id!, { balance: toMoneyString(next) });
          }
        }
        if (merged.accountId) {
          const newAccount = await db.accounts.get(merged.accountId);
          if (newAccount) {
            const next = roundMoney(Number(newAccount.balance || 0) + newDelta);
            await db.accounts.update(newAccount.id!, { balance: toMoneyString(next) });
          }
        }
      }
      ...
      if (existing.loanSettlementAccountId && existing.loanSettlementAccountId === merged.loanSettlementAccountId) {
        const account = await db.accounts.get(existing.loanSettlementAccountId);
        if (account) {
        ...
        if (existing.loanSettlementAccountId) {
          const oldAccount = await db.accounts.get(existing.loanSettlementAccountId);
          if (oldAccount) {
        ...
        if (merged.loanSettlementAccountId) {
          const newAccount = await db.accounts.get(merged.loanSettlementAccountId);
          if (newAccount) {
  ...
  async deleteTransaction(id: number): Promise<void> {
    await db.transaction('rw', [db.accounts, db.transactions], async () => {
      ...
      if (existing.settlements && existing.settlements.length > 0) {
  ...
  async addLoanSettlement(transactionId: number, settlement: LoanSettlement): Promise<Transaction> {
    return await db.transaction('rw', [db.accounts, db.transactions], async () => {
      ...
      const normalizedSettlement: LoanSettlement = {
        ...settlement,
        id: (settlement.id && settlement.id.trim().length > 0) ? settlement.id : uuidv4(),
      ...
      const account = await db.accounts.get(settlement.accountId);
      if (account) {
  ...
  async deleteLoanSettlement(transactionId: number, settlementId: string): Promise<Transaction> {
    return await db.transaction('rw', [db.accounts, db.transactions], async () => {
      ...
      const account = await db.accounts.get(settlementToDelete.accountId);
      if (account) {
  ...
  async transferBetweenAccounts(params: { fromAccountId: number; toAccountId: number; amount: string; note?: string | null; date?: Date }): Promise<void> {
    return await db.transaction('rw', [db.accounts, db.transfers], async () => {
      ...
      await db.accounts.update(fromAccountId, { balance: toMoneyString(fromBalance) });
      await db.accounts.update(toAccountId, { balance: toMoneyString(toBalance) });
